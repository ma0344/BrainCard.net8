...
namespace MyUWPApp
{
    public sealed partial class CustomInkCanvas : Grid
    {
        ...
        /// <summary>
        /// Clone strokes from this canvas into a new <see cref="InkStrokeContainer"/>.
        /// This avoids creating additional XAML UI elements (important for XAML Islands reliability).
        /// </summary>
        public InkStrokeContainer CloneStrokeContainer()
        {
        ...
        /// <summary>
        /// Clone strokes into the provided container (clears it first).
        /// </summary>
        public void CopyStrokesTo(InkStrokeContainer target)
        {
            if (target == null) throw new ArgumentNullException(nameof(target));
            target.Clear();
            foreach (var stroke in InkCanvas.InkPresenter.StrokeContainer.GetStrokes())
            {
                target.AddStroke(stroke.Clone());
            }
        }

        [Obsolete("Avoid cloning UI elements in XAML Islands. Use CloneStrokeContainer()/CopyStrokesTo() instead.")]
